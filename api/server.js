...
app.get('/api/features', (_req, res) => res.json({ riderModule: RIDER_MODULE_ENABLED }));
function requireRiderModule(req, res, next) {
  if (!RIDER_MODULE_ENABLED) return res.status(404).json({ error: 'Rider module is not enabled for this business' });
  next();
}
