...
const strip=document.getElementById('promotions-strip');if(strip){const promos=data.promotions||[];strip.innerHTML=promos.length?'<div class="promo-heading"><p class="eyebrow">RESTAURANT OFFERS</p><h2>Today\'s specials.</h2></div><div class="promo-list">'+promos.slice(0,6).map(p=>'<article><span>'+String(p.type||'OFFER').replaceAll('_',' ')+'</span><h3>'+p.name+'</h3><p>'+(p.banner_text||'Limited-time restaurant promotion')+'</p></article>').join('')+'</div>':'<div class="promo-heading"><p class="eyebrow">'+(window.TenantTheme?.name?.()||'RESTAURANT')+'</p><h2>Fresh from the kitchen.</h2></div>';}
...
updateCount();(window.TenantTheme?.ready||Promise.resolve()).then(()=>{renderMenu();renderProduct();renderCart();renderCheckout();renderOrder();renderDashboard();});
