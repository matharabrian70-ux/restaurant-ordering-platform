function renderCheckoutUpgrade(){
  ...
  document.getElementById('checkout-upgrade').onsubmit=async e=>{
    ...
    error.textContent='Creating your order…';
    try{
      ...
      if(payment.mode==='redirect' && payment.authorizationUrl){
      ...
      error.textContent=err.message || 'We could not start the payment. Please try again.';
      console.error(err);
    }
  };
}
