...
async function renderRemoteOrder(showLoading=true){
  ...
  try{
    ...
    const rider=o.rider_name?`<div class="panel"><h2>Your rider</h2><p><strong>${o.rider_name}</strong></p><p>${o.vehicle_type||'Delivery vehicle'}${o.number_plate?' · '+o.number_plate:''}</p>${o.rider_phone?`<p>${o.rider_phone}</p>`:''}<p class="muted">Delivery status: ${deliveryStatus.replaceAll('_',' ')}</p></div>`:'';
