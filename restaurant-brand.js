(function(){
  ...
  function apply(){
    ...
    if(!document.body?.classList.contains('tenant-branded')) document.body?.classList.add('tenant-branded');
    ...
    document.querySelectorAll('.brand').forEach(el=>{if(el.dataset.tenantBrandApplied===name)return;el.innerHTML=(logo?'<img class="tenant-brand-logo" src="'+esc(logo)+'" alt="">':'')+'<span class="tenant-brand-name">'+esc(name)+'</span>';el.dataset.tenantBrandApplied=name;});
    document.querySelectorAll('.manager-header h1').forEach(el=>{if(el.textContent!==name+'.')el.textContent=name+'.';});
    document.querySelectorAll('.manager-login .manager-kicker').forEach(el=>{const next='<i></i> '+esc(name.toUpperCase());if(el.innerHTML!==next)el.innerHTML=next;});
    document.querySelectorAll('.rider-auth-brand').forEach(el=>{if(el.dataset.tenantBrandApplied===name)return;el.innerHTML=(logo?'<img class="tenant-auth-logo" src="'+esc(logo)+'" alt="">':'<i></i>')+' '+esc(name.toUpperCase())+' · RIDER OPERATIONS';el.dataset.tenantBrandApplied=name;});
    document.querySelectorAll('footer').forEach(el=>{if(!el.dataset.tenantFooter)el.dataset.tenantFooter=el.textContent;const next=name+' • '+el.dataset.tenantFooter.replace(/^Savanna Bites\s*•\s*/i,'').replace(/^Restaurant management system\s*•\s*/i,'Restaurant management system • ').replace(/^Rider operations\s*•\s*/i,'Rider operations • ');if(el.textContent!==next)el.textContent=next;});
    document.querySelectorAll('.tenant-identity-slot').forEach(el=>{if(el.dataset.tenantBrandApplied===name)return;el.innerHTML='<div class="tenant-identity">'+(logo?'<img src="'+esc(logo)+'" alt="">':'')+'<div><strong>'+esc(name)+'</strong><small>Restaurant operations</small></div></div>';el.dataset.tenantBrandApplied=name;});
  }
  ...
  let observerTimer=null;let applying=false;const observer=new MutationObserver(()=>{if(applying||observerTimer)return;observerTimer=setTimeout(()=>{observerTimer=null;applying=true;try{apply();}finally{applying=false;}},50);});observer.observe(document.documentElement,{subtree:true,childList:true});
