(function(){
  ...
  function apply(next){
    ...
    document.querySelectorAll('[data-tenant-name]').forEach(el=>el.textContent=name);document.querySelectorAll('.rider-auth-brand').forEach(el=>{el.textContent=name.toUpperCase()+' · RIDER OPERATIONS';if(brand.logo_url){const img=document.createElement('img');img.src=brand.logo_url;img.alt='';img.className='tenant-auth-logo';el.prepend(img);}});document.querySelectorAll('footer').forEach(el=>{const original=el.dataset.tenantFooter||el.textContent;el.dataset.tenantFooter=original;const cleaned=original.replace(/Savanna Bites\s*•\s*/ig,'').replace(/^SAVANNA BITES\s*•\s*/i,'');el.textContent=name+' • '+cleaned;});
