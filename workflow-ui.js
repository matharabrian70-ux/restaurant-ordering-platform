...
async function renderManager(){
  ...
  try{
    ...
    const riders=features.riderModule ? await getRemoteRiders() : [];
    ...
    el.innerHTML=`<div class="dashboard-head"><div><p class="eyebrow">SAVANNA BITES • RESTAURANT</p><h1>Order control.</h1><p class="muted">Live orders, payment states and rider availability.</p></div><div><a class="btn" href="rider.html">Rider portal</a> <a class="btn" href="manager.html">Manager</a></div></div>
      ...
      ${features.riderModule ? `<section class="panel"><div class="dashboard-toolbar"><div><h2>Rider availability</h2><p class="muted">Riders are stored centrally, so different devices see the same list.</p></div><button class="btn" onclick="renderManager()">REFRESH</button></div><div class="rider-strip">${riders.length?riders.map(r=>`<div class="rider-chip"><strong>${r.name}</strong><span>${r.available?'AVAILABLE':'DELIVERING'}</span><small>${r.vehicle_type} · ${r.number_plate}</small><small>${r.id} · ${r.trip_count||0} trips</small></div>`).join(''):'<p class="muted">No riders created yet.</p>'}</div></section>` : ''}
